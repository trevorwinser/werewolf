...
$data modify storage w:temp string2 set from storage w:temp role.list[$(for_loop_start)].name
execute if function w:help/string/equals run data modify storage w:temp inner_for_loop.skip_increment set value true
$execute if data storage w:temp inner_for_loop{skip_increment:true} run data remove storage w:temp role.list[$(for_loop_start)]
