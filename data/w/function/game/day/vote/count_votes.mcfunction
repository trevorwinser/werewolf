...
execute as @a[tag=!spectator] if score @s votes = .highest votes run scoreboard players add .total_highest votes 1
...
execute if data storage w:temp state{stage:"Tie"} run tellraw @a "There was a tie! No judgement will be made."
