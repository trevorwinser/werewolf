...
execute if data storage w:temp state{time:"Night"} at @s as @a[tag=player_8,tag=!spectator,tag=!me] run loot spawn ~ ~ ~ loot w:head
...
execute at @s run tag @e[type=item,distance=..3] add renamed
tag @a remove me
schedule function w:role/menu/remove_filler 2t